...
func TestGetGarden(t *testing.T) {
	client, err := NewClient("yaml", map[string]interface{}{"filename": "testdata/gardens_data.yaml"})
	if err != nil {
...
func TestGetGardens(t *testing.T) {
	client, err := NewClient("yaml", map[string]interface{}{"filename": "testdata/gardens_end_dated.yaml"})
	if err != nil {
		t.Errorf("Unexpected error from NewClient: %v", err)
	}
