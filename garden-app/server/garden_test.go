...
func TestGetGardenWithFanSchedule(t *testing.T) {
	...
	assert.Regexp(t, `"next_fan_action":\{"time":"\d{4}-\d{2}-\d\dT\d\d:\d\d:\d\d(\.\d+)?(-07:00|Z)","is_active":(true|false),"duration_until":"[\dmhs]+"\}`, w.Body.String())
...
func TestUpdateGarden(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			nil,
			`{"fan_schedule": {}}`,
			`{"name":"test-garden","topic_prefix":"test-garden","id":"[0-9a-v]{20}","max_zones":2,"created_at":"\d{4}-\d{2}-\d\dT\d\d:\d\d:\d\d(\.\d+)?(-07:00|Z)","light_schedule":{"duration":"15h","start_time":"22:00:01-07:00"},"temperature_humidity_sensor":false,"next_light_action":{"time":"2023-08-23T13:00:01-07:00","state":"OFF"},"health":{"status":"UP","details":"last contact from Garden was 0s ago","last_contact":"2023-08-23T10:00:00Z"},"num_zones":1,"links":\[{"rel":"self","href":"/gardens/[0-9a-v]{20}"},{"rel":"zones","href":"/gardens/c5cvhpcbcv45e8bp16dg/zones"},{"rel":"action","href":"/gardens/[0-9a-v]{20}/action"}\]}`,
			http.StatusOK,
		},
		{
			"ErrorInvalidRequestBody",
